...
if [[ -n "$1" ]]; then
    PROJ_FILE="$1"
    [[ -f "$PROJ_FILE" ]] || { echo "ERROR: Specified project file not found: $PROJ_FILE"; exit 1; }
else
    # Auto-discover first *.kicad_pro in current directory
    PROJ_FILE=$(find . -maxdepth 1 -type f -name "*.kicad_pro" | head -n 1 || true)
    [[ -n "$PROJ_FILE" ]] || { echo "ERROR: No *.kicad_pro file found!"; exit 1; }
fi
